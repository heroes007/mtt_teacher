<template>
   <div class="diagnosis">
        <div class="dia-left">
            ...
            <div @click="selSeconds(i)" v-for="(t, i) in seconds" :key="i" :class="t.active&&t.num != 1 ? 'item-act' : ''" class="item item-2">
        ...
        <div class="btns">
            ...
            <div v-else class="case-types">  
                <div v-for="(t,i) in caseTypes" :key="i" >
                    <Button @click="openType(i)" v-if="t.active" class="actvie-btn" >
                        <img @click.stop="deletesType(i)" :src="deletesImg" class="deletes-img img-postion" /> {{t.title}}
                        <Icon class="icons" :size="20" :type="t.open ? 'ios-arrow-down' : 'ios-arrow-forward'" />
                    </Button>
                    <Button v-else class="btn" @click="selCaseType(i)"> <img :src="addImg" class="add-img" />  {{t.title}}</Button>
                    <div v-if="t.open&&t.active">
                        <Button @click="selType2(t, l, i)" :class="it.active ? 'act-btn' : 'de-btn'" v-for="(it,l) in t.btns" :key="l">{{it.title}}</Button>
                    </div>
                </div> 
            </div>
        </div>
        <div>
            <div class="main">
                <div v-if="showMenuPane">
                    <SceneDescription :activeSeconds="activeSeconds" page="0" v-if="page === 0" />
                    <Interrogation page="1" v-if="page == 1"  />
                    <PhysicalExam page="2" v-if="page == 2" />
                    <SupplementaryExam page="3" v-if="page == 3"  />
                    <diagnosis page="4" v-if="page == 4" />
                </div>
                <div class="menu-none" v-else>请添加病情状况</div>
            </div>
...
import SceneDescription from './com/scene-description'
...
export default {
    ...
    data() {
        return {
            ...
            casePane: 0, // 1 病情好转 2 病情恶化 3 未见好转
            caseTypes: [
                { title: '病情好转', active: false, open: false, btns: panes()},
                { title: '病情恶化', active: false, open: false, btns: panes()},
                { title: '未见好转', active: false, open: false, btns: panes()},
            ],
        }
    },
    computed: {
        showMenu(){
            return this.activeSeconds === 1
        },
        showMenuPane(){
            return this.activeSeconds === 1 || this.casePane
        }
    },
    mounted() {
    ...
    methods: {
        ...
        selType2(t, i, k){
            ...
            this.caseTypes.splice(k, 1, t)
        },
        openType(i){
            this.casePane = i + 1
            this.caseTypes.map((t, l) => {
                t.open = l === i ? !t.open : false
            })
        },
        deletesType(i){
            this.caseTypes[i].active = false
        },
        selCaseType(i){
            this.caseTypes[i].active = true
        },
        selSeconds(i){
